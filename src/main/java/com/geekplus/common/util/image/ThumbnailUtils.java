...
//            InputStream inStream = conn.getInputStream();
    ...
    //转变base64图片缩略图
    public static String getThumbnailBase64Image(String baseDir, String imgPath){
        String base64Str="";
        String serverImgPath;
        try {
            //geekplus是图片的全网址是否含有的网站域名(例如：geekplus.xx)，Constant.RESOURCE_PREFIX是后端静态资源访问映射的地址(http://geekplus.xx)
    ...
    public static String getThumbnailImage(String imgPath){
        String thumbnailImg="";
        String serverImgPath;
        String suffixExtension=".png";
        try {
            // geekplus是图片的全网址是否含有的网站域名，Constant.RESOURCE_PREFIX是后端静态资源访问映射的地址
            if(imgPath.contains("geekplus") || imgPath.startsWith(Constant.RESOURCE_PREFIX)){
                ...
                suffixExtension = getThumbnailSuffixExtension(serverImgPath);
                BufferedImage originalImage = ImageIO.read(new File(serverImgPath));
                String thumbnailPath=File.separator+"thumbnail"+File.separator + DateUtil.datePath() + File.separator + UUIDUtil.getUUIDTimeStamp() + suffixExtension;
                //String serverThumbnailPath=WebAppConfig.getProfile()+thumbnailPath;
                File serverDesc = FileUploadUtils.getAbsoluteFile(WebAppConfig.getUploadPath(), thumbnailPath);
                if(ImageUtil.calculateSharpness(originalImage)>=60) {
                    // 生成缩略图 inputStream
                    Thumbnails.of(new File(serverImgPath))
                            .scale(0.4d)
...
//                thumbnailImg=Constant.RESOURCE_PREFIX+thumbnailPath;
    ...
    public static void main(String[] args) throws IOException {
        String imgPath="https://www.xxx.xxx/article/2023/4ec058f6-adfb-803d44120ec5.png";
