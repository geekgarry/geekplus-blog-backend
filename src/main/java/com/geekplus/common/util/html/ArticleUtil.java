package com.geekplus.common.util.html;

import com.geekplus.common.util.encrypt.SignatureUtil;
import com.geekplus.common.util.image.ThumbnailUtils;
import com.geekplus.common.util.string.StringUtils;
import com.geekplus.webapp.function.controller.GeekPlusAppController;

import java.util.*;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

/**
 * author     : geekplus
 * date       : 3/26/23 22:03
 * description:
 */
public class ArticleUtil {

//    private static final String[] TAGS = {"img", "audio", "video", "a"};
//    private static final String TAG_PATTERN = String.join("|", Arrays.stream(TAGS).map(t -> "<" + t + "\\b([^>]*?)>").toArray(String[]::new));
//    private static final Pattern PATTERN = Pattern.compile(TAG_PATTERN,Pattern.CASE_INSENSITIVE);
    private static final Pattern TAG_PATTERN =
            Pattern.compile("<(img|audio|video|a)\\b([^>]*?)>",
                    Pattern.CASE_INSENSITIVE);

    public static String processRichText(String content) {
        SignatureUtil signer = new SignatureUtil();
        Matcher matcher = TAG_PATTERN.matcher(content);
        StringBuffer sb = new StringBuffer();
        while (matcher.find()) {
            String tagName = matcher.group(1).toLowerCase(); // img, audio, video, a
            String attrs = matcher.group(2);                 // 所有属性字符串
            System.out.println(tagName+":"+attrs);
            // 解析 src / href
            String url = extractUrl(attrs, tagName);

            String signedUrl = null;
            if (StringUtils.isNotEmpty(url)) {
                // 调用你的后端签名方法
                signedUrl = signer.signedUrl(url);
            }

            String newTag;

            if ("a".equals(tagName)) {
                newTag = rebuildATag(attrs, signedUrl);
            } else {
                newTag = rebuildMediaTag(tagName, attrs, signedUrl);
            }

            matcher.appendReplacement(sb, Matcher.quoteReplacement(newTag));
        }

        matcher.appendTail(sb);
        return sb.toString();
    }

    /**
      * 提取标签中的 src 或 href
      */
    private static String extractUrl(String attrs, String tagName) {
        Matcher m;

        if ("a".equals(tagName)) {
            m = Pattern.compile("href\\s*=\\s*\"([^\"]+)\"").matcher(attrs);
        } else {
            m = Pattern.compile("src\\s*=\\s*\"([^\"]+)\"").matcher(attrs);
        }

        return m.find() ? m.group(1) : null;
    }

    /**
      * 构建新的 media 标签（img/audio/video）
      */
    private static String rebuildMediaTag(String tagName, String attrs, String signedUrl) {
        if (signedUrl == null) return "<" + tagName + attrs + ">";

        String newAttrs = attrs.replaceAll(
                "src\\s*=\\s*\"([^\"]+)\"",
                "src=\"" + signedUrl + "\""
        );

        return "<" + tagName + newAttrs + ">";
    }

    /**
      * 构建新的 a 标签（确保 target="_blank"）
      */
    private static String rebuildATag(String attrs, String signedUrl) {
        String newAttrs = attrs;

        // 替换 href
        if (signedUrl != null) {
            newAttrs = newAttrs.replaceAll(
                    "href\\s*=\\s*\"([^\"]+)\"",
                    "href=\"" + signedUrl + "\""
            );
        }

        // 增加 target="_blank"
        if (!newAttrs.toLowerCase().contains("target=")) {
            newAttrs += " target=\"_blank\"";
        }

        return "<a" + newAttrs + ">";
    }

    /**
     * 得到网页中图片的地址，获取html内容中所有图片集合
     * @param htmlStr html字符串
     */
    public static Set<String> getImgStr(String htmlStr) {
        Set<String> pics = new HashSet<>();
        String img = "";
        Pattern p_image;
        Matcher m_image;
        //     String regEx_img = "<img.*src=(.*?)[^>]*?>"; //图片链接地址
        String regEx_img = "<img.*src\\s*=\\s*(.*?)[^>]*?>";
        p_image = Pattern.compile
                (regEx_img, Pattern.CASE_INSENSITIVE);
        m_image = p_image.matcher(htmlStr);
        while (m_image.find()) {
            // 得到<img />数据 不懂的qq1023732997
            img = m_image.group();
            // 匹配<img>中的src数据
            Matcher m = Pattern.compile("src\\s*=\\s*\"?(.*?)(\"|>|\\s+)").matcher(img);
            while (m.find()) {
                pics.add(m.group(1));
            }
        }
        return pics;
    }

    public static String getOneThumbnail(String articleContent){
        String thumbnailImage="";
        Set<String> fp = ArticleUtil.getImgStr(articleContent);
        String[] fps = fp.toArray(new String[0]);
        if (!fp.isEmpty()) {
            Random random=new Random();
            int imgIndex=random.nextInt(fps.length);
            //System.out.println("文章第一张图片image："+fp.iterator().next());//获取第一张
            thumbnailImage = ThumbnailUtils.getThumbnailImage(fps[imgIndex]);
            //thumbnailImage = ThumbnailUtils.getThumbnailBase64Image(fp.iterator().next());
        }
        return thumbnailImage;
    }

    /**
     * 得到网页中图片的地址
     * @param htmlStr html字符串
     */
    public static List<String> getImgSrc(String htmlStr) {

        if( htmlStr == null ){

            return null;
        }

        String img = "";
        Pattern p_image;
        Matcher m_image;
        List<String> pics = new ArrayList<String>();

        String regEx_img = "<img.*src\\s*=\\s*(.*?)[^>]*?>";
        p_image = Pattern.compile(regEx_img, Pattern.CASE_INSENSITIVE);
        m_image = p_image.matcher(htmlStr);
        while (m_image.find()) {
            img = img + "," + m_image.group();
            // Matcher m =
            // Pattern.compile("src=\"?(.*?)(\"|>|\\s+)").matcher(img); //匹配src
            Matcher m = Pattern.compile("src\\s*=\\s*\"?(.*?)(\"|>|\\s+)").matcher(img);

            while (m.find()) {
                pics.add(m.group(1));
            }
        }
        return pics;
    }

    //生成文章摘要
    public static String getArticleAbstract(String htmlArticleComment){

        String regex = "\\s+";
        String str = htmlArticleComment.trim();
        //去掉所有空格
        String articleTabloid = str.replaceAll(regex,"");

        int beginIndex = articleTabloid.indexOf("<");
        int endIndex = articleTabloid.indexOf(">");
        String myArticleTabloid = "";
        String nowStr = "";
        while (beginIndex != -1){
            nowStr = articleTabloid.substring(0, beginIndex);
            if(nowStr.length() > 197){
                nowStr = nowStr.substring(0,197);
                myArticleTabloid += nowStr;
            } else {
                myArticleTabloid += nowStr;
            }
            articleTabloid = articleTabloid.substring(endIndex + 1);
            beginIndex = articleTabloid.indexOf("<");
            if(myArticleTabloid.length() < 197){
                //过滤掉<pre>标签中的代码块
                if(articleTabloid.length() > 4){
                    if(articleTabloid.charAt(beginIndex) == '<' && articleTabloid.charAt(beginIndex+1) == 'p'  && articleTabloid.charAt(beginIndex+2) == 'r'  && articleTabloid.charAt(beginIndex+3) == 'e' ){
                        endIndex = articleTabloid.indexOf("</pre>");
                        endIndex = endIndex + 5;
                    } else {
                        endIndex = articleTabloid.indexOf(">");
                    }
                } else {
                    endIndex = articleTabloid.indexOf(">");
                }
            } else {
                break;
            }
        }
        if(myArticleTabloid.length() > 197){
            myArticleTabloid = myArticleTabloid.substring(0, 197);
        }
        return myArticleTabloid;
    }

    public static void main(String[] args) {
        String content="";//"<img src=\"../profile/article/2024/12/20/R2Vla1BsdXNjZGZkZjA1Ni1iM2MyLTRhMjUtOGMwZC00ODMxNGJjZjY1NmZGcmkgRGVjIDIwIDAwOjQ1OjU5IENTVCAyMDI0.png\" alt=\"基于 java 注册登录 MD5 算法加盐加密 &amp;&amp; 颁发 Token 身份令牌\">";
        System.out.println(processRichText(content));
    }
}
