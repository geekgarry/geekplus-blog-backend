...
//    }
    ...
    public Object method(ProceedingJoinPoint pjp) throws Throwable {
        ...
        List<SysRoleVO> sysRoles=principal1.getSysRoleList();
        List<String> roleKey= sysRoles.stream().map(SysRoleVO::getRoleKey).collect(Collectors.toList());
        List roleSet=Arrays.asList("admin", "webManage", "common");
        //判断用户是否为限制访问用户，在规定时间内最大请求数
        if(roleKey.containsAll(roleSet)) {
            if (principal1 == null) {
                limitValue = principal1.getUsername();
            } else {
                limitValue = ip;
            }
        }
