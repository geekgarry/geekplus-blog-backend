...
/**
  * @Author geekplus
  * @Description //结合jwt实现继承BasicHttpAuthenticationFilter过滤器，重写方法
  */
...
//                }
                handleLoginException(response,new AuthenticationException("登录状态过期失效，认证失败！"));//"token验证失败，无效操作，登录权限不足"
                return false;
            }
        }
...
//        }
    ...
    private void handleLoginException(ServletResponse response, AuthenticationException e) {
        ...
        try(PrintWriter writer = response.getWriter()){
            Result resultVo=Result.error(HttpStatus.FORBIDDEN,e.getMessage());
