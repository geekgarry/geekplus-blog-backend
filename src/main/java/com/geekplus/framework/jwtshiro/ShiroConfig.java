...
//    }
    ...
    //权限管理，配置主要是Realm的管理认证 DefaultWebSecurityManager也可以
    @Bean
...
//        filterRegistrationBean.addInitParameter("targetFilterLifecycle", "true");
    ...
    /**
     * DefaultAdvisorAutoProxyCreator 是一个BeanPostProcessor
     * 负责在Spring容器启动时查找所有Advisor类型的Bean，并为符合条件的Bean创建代理
     * 启用Shiro注解，如@RequiresPermissions等，通过Advisor来拦截方法，然后判断是否具有权限
     * 如果不配置此Bean，那么Shiro的注解将不会生效
     * 开启shiro授权注解，若上面Bean未生效则使用此Bean
     */
    @Bean
    //@ConditionalOnMissingBean(DefaultAdvisorAutoProxyCreator.class) // 可以考虑加上这个条件注解
    @DependsOn("lifeCycleBeanPostProcessor")
    public DefaultAdvisorAutoProxyCreator defaultAdvisorAutoProxyCreator(){
        ...
        defaultAAPC.setAdvisorBeanNamePrefix("_no_advisor");
        return defaultAAPC;
    }
