...
public interface ResumeTemplateMapper {
    ...
    @Insert("INSERT INTO resume_template(`key`, name, description, layout_json) VALUES(#{key}, #{name}, #{description}, #{layoutJson})")
    int insert(ResumeTemplate template);

    @Update("UPDATE resume_template SET `key` = #{key}, name = #{name}, description = #{description}, layout_json = #{layoutJson} WHERE id = #{id}")
