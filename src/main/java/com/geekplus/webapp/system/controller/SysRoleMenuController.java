...
public class SysRoleMenuController extends BaseController {
    ...
    public Result add(@RequestBody SysRoleMenu sysRoleMenu) {
        if(sysRoleMenuService.insertSysRoleMenu(sysRoleMenu)>0){
            ...
            Set sysUserPerms = sysUserService.getSysUserMenuPerms(loginUser.getUserId());
    ...
    */
     ...
     * 删除系统用户角色关系表
     */
    @Log(title = "删除角色和权限关系",businessType = BusinessType.DELETE)
     ...
     * 删除系统用户角色关系表
     */
    @Log(title = "批量删除角色和权限关系",businessType = BusinessType.DELETE)
    @PutMapping("/batchDeleteRoleMenu")
    public Result removeRoleMenuList(@RequestBody List<SysRoleMenu> sysRoleMenuList) {
        if(sysRoleMenuService.batchDeleteSysRoleMenu(sysRoleMenuList)>0) {
            LoginUser loginUser = sysUserTokenService.getLoginUser(ServletUtil.getRequest());
            Set sysUserPerms = sysUserService.getSysUserMenuPerms(loginUser.getUserId());
            loginUser.setSysMenuList(sysUserPerms);
            sysUserTokenService.setLoginUser(loginUser);
