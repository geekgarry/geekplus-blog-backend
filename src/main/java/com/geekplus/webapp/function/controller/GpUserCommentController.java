...
public class GpUserCommentController extends BaseController
{
     ...
     * 新增用户评论回复留言
     */
    @Log(title = "新增用户评论回复留言", businessType = BusinessType.INSERT)
    @PostMapping
    public Result add(@RequestBody GpUserComment gpUserComment)
    {
        return toResult(gpUserCommentService.insertGpUserComment(gpUserComment));
    }

    /**
     * 修改用户评论回复留言
     */
    @Log(title = "修改用户评论回复留言", businessType = BusinessType.UPDATE)
    @PutMapping
     ...
     * 删除用户评论回复留言
     */
    @Log(title = "删除用户评论回复留言", businessType = BusinessType.DELETE)
	@DeleteMapping("/{ids}")
    public Result remove(@PathVariable Long[] ids)
    {
        return toResult(gpUserCommentService.deleteGpUserCommentByIds(ids));
    }
}
