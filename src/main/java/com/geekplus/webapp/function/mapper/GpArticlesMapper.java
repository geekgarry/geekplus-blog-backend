package com.geekplus.webapp.function.mapper;

import com.geekplus.webapp.function.entity.GpArticles;
import org.apache.ibatis.annotations.Param;

import java.util.List;

/**
 * 文章Mapper接口
 *
 * @author 佚名
 * @date 2023-03-12
 */
public interface GpArticlesMapper
{
    /**
     * 查询文章
     *
     * @param id 文章ID
     * @return 文章
     */
    public GpArticles selectGpArticlesById(Long id);

    /**
     * @Author geekplus
     * @Description //前端网站查询文章详情
     * @Param
     * @Throws
     * @Return {@link }
     */
    public GpArticles selectGpArticlesByIdForUser(String isDisplay, Long id);

    /**
      * @Author geekplus
      * @Description //前端网站查询文章详情
      * @Param
      * @Throws
      * @Return {@link }
      */
    public GpArticles selectGpArticlesByIdForWeb(Long id);

    /**
      * @Author geekplus
      * @Description //随机推荐文章
      * @Param
      * @Throws
      * @Return {@link }
      */
    List<GpArticles> selectGpArticlesByRandom(Long articleCount);

    //固定查询浏览量最多的前几条
    List<GpArticles> selectMostViewedGpArticlesListByLimit(Integer limitCount);
    //动态条件查询某目录时间最新的前几条
    List<GpArticles> selectGpArticlesListByCategoryLimit(String pathName, Integer limitCount);
    //查询包含在id数组中的所有文章列表
    int selectGpArticlesListByIdList(Long[] ids);
    //根据目录ID查询分页的文章
    List<GpArticles> selectGpArticlesListByCategory(String pathName);
    //根据搜索标题动态查询分页的文章
    List<GpArticles> selectGpArticlesListByKeyWords(String articleTitle);
    //根据目录ID查询首页大屏的一篇文章
    GpArticles selectOneArticleByCategory(String pathName);
    //根据目录ID查询首页大屏的四篇文章
    List<GpArticles> selectFourArticleByCategory(String pathName);

    /**
     * 查询文章列表
     *
     * @param gpArticles 文章
     * @return 文章集合
     */
    public List<GpArticles> selectGpArticlesList(GpArticles gpArticles);

    /**
     * 给前端博客用户查询文章列表
     *
     * @param gpArticles 文章
     * @return 文章集合
     */
    public List<GpArticles> selectGpArticlesListForUser(GpArticles gpArticles);

    /**
     * 新增文章
     *
     * @param gpArticles 文章
     * @return 结果
     */
    public int insertGpArticles(GpArticles gpArticles);

    /**
      * @Author geekplus
      * @Description //用户投稿文章
      * @Param
      * @Throws
      * @Return {@link }
      */
    public int userWriteGpArticles(GpArticles gpArticles);

    /**
     * 修改文章
     *
     * @param gpArticles 文章
     * @return 结果
     */
    public int updateGpArticles(GpArticles gpArticles);

    /**
      * @Author geekplus
      * @Description //用于用户端更新浏览量和点赞数
      * @Param
      * @Throws
      * @Return {@link }
      */
    public int updateGpArticlesForUser(GpArticles gpArticles);

    /**
     * 删除文章
     *
     * @param id 文章ID
     * @return 结果
     */
    public int deleteGpArticlesById(Long id);

    /**
     * 批量删除文章
     *
     * @param ids 需要删除的数据ID
     * @return 结果
     */
    public int deleteGpArticlesByIds(Long[] ids);

    /**
      * @Author geekplus
      * @Description //根据标签ID查询文章列表,查询这个tag_id的文章列表
      * @Param
      * @Throws
      * @Return {@link }
      */
    public List<GpArticles> selectTagDeArticle(@Param("tagName") String tagName, @Param("tagId") Long tagId);

    /**
      * @Author geekplus
      * @Description //查找当前文章的上一篇
      * @Param
      * @Throws
      * @Return {@link }
      */
    public GpArticles selectPrevGpArticle(@Param("pathName") String pathName, @Param("articleId") Long articleId);

    /**
     * @Author geekplus
     * @Description //查找当前文章的下一篇
     * @Param
     * @Throws
     * @Return {@link }
     */
    public GpArticles selectNextGpArticle(@Param("pathName") String pathName, @Param("articleId") Long articleId);
}
