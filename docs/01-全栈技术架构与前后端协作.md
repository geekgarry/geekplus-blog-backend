...
### 2.1.1 列表动态条件查询（与 Security-JWT 同套）

前端 `DynamicQueryForm` → GET **`conditionsJson`** 字符串 → 后端 `DynamicQueryHelper.prepare` → `params.dq` → MyBatis `DynamicWhere`。  
并复用 `BaseEntity` 的时间区间 / 排序 / 关键词字段。用户列表已接入；详情与接入步骤见 [17-动态条件查询.md](./17-动态条件查询.md)。
...
<!-- | `/geekplusapp/getSiteThemeColor` | 全站主题色（暂未启用，见前端 siteTheme.js） | -->
